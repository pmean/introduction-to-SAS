...
```{r m03-5507-simon-import-space-delimited.sas}
px <- function(tx, s1, s2, xtra_lines=0) {
  tx %>% str_which(s1) -> i1
  tx %>% str_which(s2) -> j1
  if (length(i1)==0) return("No beginning found")
  if (length(j1)==0) return("No ending found")
  if (length(i1) > 1) return(paste("Multiple beginnings found:", head(i1)))
  if (length(j1) > 1) return(paste("Multiple endings found:", head(j1)))
  j1 <- j1 + xtra_lines
  tx %>%
    extract(i1:j1) %>%
    paste0(collapse="\n") %>%
...
opts_current$get("label") %>%
  readLines(warn=FALSE) -> tx
tx %>% px("^\\* m03", "license") %>% cat
```
...
```{r m03-5507-simon-import-comma-delimited.sas}
opts_current$get("label") %>%
  readLines(warn=FALSE) -> tx
tx %>% px("^\\* m03", "license") %>% cat
```
...
```{r m03-5507-simon-import-tab-delimited.sas}
opts_current$get("label") %>%
  readLines(warn=FALSE) -> tx
tx %>% px("^\\* m03", "license") %>% cat
```
...
```{r m03-5507-simon-import-tilde-delimited.sas}
opts_current$get("label") %>%
  readLines(warn=FALSE) -> tx
tx %>% px("^\\* m03", "license") %>% cat
```
...
```{r m03-5507-simon-import-fixed-width.sas}
opts_current$get("label") %>%
  readLines(warn=FALSE) -> tx
tx %>% px("^\\* m03", "license") %>% cat
```
...
```{r m03-5507-simon-import-string-data.sas}
opts_current$get("label") %>%
  readLines(warn=FALSE) -> tx
tx %>% px("^\\* m03", "license") %>% cat
```
...
```{r m03-5507-simon-import-first-line-names.sas}
opts_current$get("label") %>%
  readLines(warn=FALSE) -> tx
tx %>% px("^\\* m03", "license") %>% cat
```

...
px <- function(tx, s1, s2, xtra_lines=0) {
  tx %>% str_which(s1) -> i1
  tx %>% str_which(s2) -> j1
...
```{}
`r px(tx, fixed("* m02-"), fixed("* license"))`
```
...
Here's how you can simulate blanks without actually using blanks.
  ...
  * the dot (period). (Note that SAS does not allow you to use the perior this way.)
...
```{}
...
`r px(tx, "^    neck=", "styloid processes", 2)`
...
```{}
`r px(tx, "Calculate simple statistics", "unusual minimum value", 1)`
```
...
```{}
`r px(tx, "Look at smallest value", "inconsistency with wt", 1)`
```
...
```{}
`r px(tx, "Look at largest value", "normal to me", 1)`
```
...
```{}
`r px(tx, "Faulty approach", "0 will include ht", 1)`
```
...
### SAS output, proc print wiht faulty logic
...
```{}
`r px(tx, "Counting missing values", "nmiss statistic", 1)`
```
...
```{}
`r px(tx, "Simple transformations", "Display a histogram", -2)`
```
...
```{}
`r px(tx, "Display a histogram", "Revised histogram with narrow bins", -2)`
```
...
```{}
`r px(tx, "Revised histogram with narrow bins", "Note to myself", -2)`
```
...
```{}
`r px(tx, "Calculate correlations", "Save the correlations", -2)`
```
...
```{}
`r px(tx, "Save the correlations", "Modifying these correlations", -2)`
```
...
```{}
`r px(tx, "Modifying these correlations", "Draw a scatterplot", -2)`
```
...
```{}
`r px(tx, "Draw a scatterplot", "Adding linear", -2)`
```
...
```{}
`r px(tx, "Adding linear", "Adding a smooth curve", -2)`
```
...
```{}
`r px(tx, "Adding a smooth curve", "with a smooth curve", 1)`
```

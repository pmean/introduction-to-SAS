...
add_chunk <- function(i, tx=tx) {
  ...
  if (i>length(tx)) {return("###" %0% i %0% " out of bounds.")}

  if(str_detect(tx[i], "^* Part")) {
    ti <<- tx[[i]] %>% 
      str_remove("^\\* Part..\\. ") %>%
      str_remove(";$")
    return("\n\n")
  ...
  if(str_detect(tx[i], "^* Notes")) {
    tx[i] %>%
      str_remove("^\\* ") -> md
    '<div class="notes"> ' %0% i %2%
    md %2%
    '</div>\n\n' -> md
    return(md)
  }

  if(str_detect(tx[i], "Output, page")) {
